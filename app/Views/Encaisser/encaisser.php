...
<main>
    ...
    <form id="selectCommandeForm" method="get" action="<?php url_to('Encaisser_Form') ?>">
        <label>Sélectionnez une commande:
            <select name="commandeID" onchange="document.getElementById('selectCommandeForm').submit()">
                ...
                <?php foreach ($commandes as $commande) : ?>
                    <option value="<?= $commande['COMMANDEID'] ?>" <?= $selectedCommandeID == $commande['COMMANDEID'] ? 'selected' : '' ?>><?= $commande['DATE_HEURE'] ?></option>
                <?php endforeach; ?>
            </select>
        </label>
    </form>
